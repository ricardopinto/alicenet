...
func makeTransfer(sender objs.Signer, receiver objs.Signer, transferAmount uint64, v *objs.TXOut) (*objs.Tx, error) {
	txIn, err := v.MakeTxIn()
	if err != nil {
		return nil, err
	}
...
func createNonExistentUTXO(chainID uint32, value *uint256.Uint256, senderPubkey []byte, txHash []byte, txOutIdx uint32) (*objs.TXOut, error) {
	newValueStoreSender := &objs.ValueStore{
		VSPreImage: &objs.VSPreImage{
			ChainID:  chainID,
			Value:    value,
...
func createBadTx(sender objs.Signer, receiver objs.Signer, transferAmount uint64) (*objs.Tx, error) {
	pubKey, err := sender.Pubkey()
	if err != nil {
		return nil, err
	}
	value := &uint256.Uint256{}
	value, err = value.FromUint64(transferAmount * 2)
	if err != nil {
		return nil, err
	}
	...
	tx, err := makeTransfer(sender, receiver, transferAmount, nonExistingUTXO)
	if err != nil {
		return nil, err
	}
...
func (tm *txHandler) GetTxsForProposal(txn *badger.Txn, chainID, height uint32, curveSpec constants.CurveSpec, signer objs.Signer, maxBytes uint32) (objs.TxVec, []byte, error) {
	...
	// 	return nil, nil, errorz.ErrInvalid{}.New(err.Error())
	// }
	// myAddr := crypto.GetAccount(pubKey)
	os, err := tm.cdb.GetOwnState(txn)
	if err != nil {
		utils.DebugTrace(tm.logger, err)
		return nil, nil, errorz.ErrInvalid{}.New(err.Error())
	}
	vs, err := tm.cdb.GetValidatorSet(txn, height)
	if err != nil {
		utils.DebugTrace(tm.logger, err)
		return nil, nil, errorz.ErrInvalid{}.New(err.Error())
	}
	tm.logger.WithFields(logrus.Fields{
		"os.VAddr":               os.VAddr,
		"vs.Validators[0].VAddr": vs.Validators[0].VAddr,
	}).Warnf("should i do bad stuff?")

	if int(height) >= badHeight && bytes.Equal(vs.Validators[0].VAddr, os.VAddr) {
		tm.logger.WithFields(logrus.Fields{
			"os.VAddr": os.VAddr,
		}).Warnf("hey I'm doing a bad transaction")
		//var badTx interfaces.Transaction
		badTx, err := createBadTx(signer, signer, 1)
		if err != nil {
			utils.DebugTrace(tm.logger, err)
			return nil, nil, err
		}
	...
	_, _, spent, err := tm.dHdlr.Get(txn, consumedDeposits)
	if err != nil {
		utils.DebugTrace(tm.logger, err)
		return nil, nil, err
	}
	tm.logger.Warn("aa")
	// todo: delete this IF
	// if !bytes.Equal(vs.Validators[0].VAddr, os.VAddr) {
	// if len(missing) > 0 {
